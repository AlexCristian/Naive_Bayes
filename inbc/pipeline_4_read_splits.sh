...
fq_dir=test_set
start=`date +%s`
for file in $(ls $fq_dir/*.fna)
do
prefix=${file%.fna}
number=$(echo $prefix | cut -d '_' -f4)
if [ "$number" -ge 28 ]
then
echo output file in dir: $prefix/test
python split_reads.py $file $prefix/test
fi
done
end=`date +%s`
runtime=$((end-start))
echo "converting time is $runtime"
